...
class Client:
    ...
    def __init__(self, grpc_addr="localhost:50052", device_id=0):
        ...
        client_id = 0
        try:
            self.interface = gc.ClientInterface(grpc_addr=grpc_addr, client_id=client_id, device_id=device_id, num_tries=NUM_TRIES)
    ...
    def program_table(self, table_name, table_index):
        #table = self._get_table(table_name)
        #key = table.make_key([gc.KeyTuple("$METER_INDEX", table_index)])
        pass
    ...
    def program_meter(self, meter, meter_index, meter_type, cir, pir, cbs, pbs):
        ...
        try:
            ...
            self.dump_table(table_name=meter, key_name="$METER_INDEX", key_value=meter_index)
        except Exception:

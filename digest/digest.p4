...
control Ingress(inout header_t hdr, inout metadata_t meta,
  ...
  action ipv4_forward(macaddr_t dstAddr, egress_spec_t port) {
    ...
    meta.tmp_digest.id = rnd_generator.get();
    meta.tmp_digest.srcAddr = hdr.ipv4.srcAddr;
    meta.tmp_digest.dstAddr = hdr.ipv4.dstAddr;
...
IngressDeparser(packet_out pkt, inout header_t hdr, in metadata_t meta,
  ...
  apply {
    ...
    if (ig_dprsr_md.digest_type == 1) {
        digest_inst.pack({meta.tmp_digest.id, meta.tmp_digest.srcAddr, meta.tmp_digest.dstAddr});
    }
